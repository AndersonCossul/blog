<fieldset class="form-group {{ $errors->has('title') ? ' has-error' : '' }}">
    ...
    <input name="title" type="text" required placeholder="Title" class="form-control" value="{{ old('title') ?: @$post->title }}">
</fieldset>
    ...
    <select name="category_id" class="form-control" required>
        ...
        @foreach ($categories as $category)
            <option value="{{ $category->id }}"
                    {{ (old('category_id') == $category->id || @$post->id == $category->id) ? 'selected' : '' }}>
                {{ $category->name }}
            </option>
        @endforeach
    </select>
